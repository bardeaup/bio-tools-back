...
public class CellCountExperimentDS {
    ...
    @Transactional
    public Page<Experiment> loadExperimentHistory(int page, int size, Optional<String> experimentName,
												  Optional<String> cells, Optional<String> treatment) throws DataNotFoundException{
        UserPrinciple principal = (UserPrinciple) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        PageRequest pageRequest = PageRequest.of(page, size);
		Page<Experiment> experimentPage = this.experimentRepo.findAll(principal.getId(), experimentName.orElse("_"),
                cells.orElse("_"), treatment.orElse("_"), pageRequest);
        if(experimentPage == null){
        	throw new DataNotFoundException("No experiment found corresponding to parameters : experimentName : "
					+ experimentName +" , cells : "+ cells +" , treatment : "+ treatment );
		}
     ...
     *
     * @param countListForAnalysisDTO of CountForAnalysisDTO with initial and final date and cell quantity
